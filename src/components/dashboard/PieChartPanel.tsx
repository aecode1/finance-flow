          ...
          >
            {(['expense', 'income'] as TransactionType[]).map(tab => (
              <button
                ...
                className="relative px-4 py-1.5 rounded-lg text-xs font-sans font-semibold transition-colors duration-200 z-10"
                style={{
                  color: activeTab === tab ? '#fff' : '#4A5C80',
                }}
              >
                {activeTab === tab && (
                  <motion.span
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                      <AnimatePresence mode="wait">
                        {activeIndex !== undefined ? (
                          ...
                          >
                            <span className="block text-[10px] font-sans uppercase tracking-wider" style={{ color: chartData[activeIndex]?.color }}>
                              {chartData[activeIndex]?.name}
                            </span>
                            <span className="block font-display text-base font-bold text-[#F0F4FF] leading-tight mt-0.5">
                              {formatCurrency(chartData[activeIndex]?.value ?? 0)}
                            </span>
                            <span className="block text-[10px] font-mono mt-0.5" style={{ color: '#4A5C80' }}>
                          ...
                          <motion.div
                            ...
                            className="text-center"
                          >
                            <span className="block text-[10px] font-sans uppercase tracking-widest text-[#4A5C80]">toplam</span>
                            <span className="block font-display text-base font-bold text-[#F0F4FF] leading-tight mt-0.5">
                              {formatCurrency(grandTotal)}
                            </span>
                            <span className="block text-[10px] font-sans text-[#4A5C80] mt-0.5">
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className="text-xs font-sans font-medium text-[#F0F4FF]">{entry.name}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-mono" style={{ color: entry.color }}>
                              %{pct.toFixed(1)}
                            </span>
                            <span className="text-xs font-mono text-[#8B9DC3]">{formatCurrency(entry.value)}</span>
                          </div>
                        </div>
                        {/* Progress bar */}
